...
) : ControllerBase
{
    ...
    public async Task<IActionResult> GetByWorkbook(Guid workbookId)
    {
        return Ok(await getSheetsByWorkbookUseCase.ExecuteAsync(workbookId));
    }

    [HttpGet("sheets/{sheetId:guid}")]
    public async Task<IActionResult> GetSheet(Guid sheetId)
    {
        return Ok(await getSheetDetailUseCase.ExecuteAsync(sheetId));
    }
